...
export const register = async (req: Request, res: Response) => {
  ...
  var argsString =  req.body.password;
...
export const login = async (req: Request, res: Response) => {
 ...
 con.query('SELECT * FROM tron_user  WHERE email = ? ',[email], function (error, data) {
   if (error) {
    ...
    var phpScriptPath = "C:/wamp64/www/script1.php";
    var argsString = [ req.body.password, data[0].password_hash];
  ...
  var pass = phpResponse;
     ...
     if(data.length >0){
       ...
       console.log(data[0].password_hash);
         ...
         if(phpResponse==data[0].password_hash){
             res.status(200).json({
                 status:true,
